...
const {getWalletBalance,transferSOL,airDropSol,airDropSol2}=require("./solana");
...
const gameExecution=async ()=>{
    init();
    //AirDrop 10SOL to userWallet ( so he can play )
    user_balance = await getWalletBalance(userWallet.publicKey);
    console.log("User balance : ", user_balance);

    //await airDropSol(userWallet,10);
    await airDropSol2(userWallet.publicKey, 10);
    ...
    if(answers.RANDOM){
        ...
        if(answers.RANDOM===generateRandomNumber){
            //AirDrop Winning Amount to treasuryWallet ( to be able to pay )
